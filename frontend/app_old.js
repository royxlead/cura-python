...
class APIService {
    ...
    async sendMessage(message, options = {}) {
        // Check if we're in guest mode and handle appropriately
        if (this.token === 'guest_token') {
    ...
    setGuestToken() {
        // Set a special guest token identifier
...
class ChatManager {
    ...
    async sendMessage() {
        ...
        try {
            ...
            if (error.message === 'guest_auth_required') {
                errorMessage = `
                    <div class="guest-auth-message">
                        <h4>🔐 Authentication Required</h4>
                        <p>To use CURA's full AI capabilities, you'll need to create an account or sign in.</p>
                        <div class="auth-buttons">
                            <button onclick="app.showAuthInterface(); app.showView('signup')" class="btn btn-primary btn-sm">
                                Create Account
                            </button>
                            <button onclick="app.showAuthInterface(); app.showView('login')" class="btn btn-secondary btn-sm">
                                Sign In
                            </button>
                        </div>
                        <p><small>Your account will be secure and HIPAA compliant.</small></p>
                    </div>
                `;
                this.addMessage(errorMessage, 'ai-auth-required');
                this.notifications.info('Please create an account to continue chatting with CURA', {
    ...
    formatMessageContent(content, sender) {
        ...
        if (sender === 'ai-auth-required') {
            // Return the content as-is since it already contains HTML
...
// Main Application Class
    ...
    async handleSignup(e) {
        ...
        if (!userData.username || userData.username.length < 3) {
            this.notifications.error('Username must be at least 3 characters long.');
            return;
        }

        if (!userData.password || userData.password.length < 8) {
            this.notifications.error('Password must be at least 8 characters long.');
            return;
        }
...
class CuraApp {
    constructor() {
        ...
        this.auth = new AuthManager(this.api, this.notifications);
        this.chat = null;
    ...
    async init() {
        ...
        const isAuthenticated = authToken && authToken !== 'guest_token';
        
        if (isAuthenticated) {
            console.log('App: User is authenticated, loading user profile...');
            try {
                const userProfile = await this.api.getUserProfile();
                this.setUser(userProfile);
                this.showChatInterface();
            } catch (error) {
                console.log('App: Failed to load user profile, showing auth interface...');
                this.showAuthInterface();
            }
        } else {
            console.log('App: User not authenticated, showing welcome screen...');
            this.showAuthInterface();
    ...
    setupEventListeners() {
        // Get started button - navigate directly to chat
        const getStartedBtn = document.getElementById('getStartedBtn');
        if (getStartedBtn) {
            getStartedBtn.addEventListener('click', () => {
                console.log('Get Started button clicked - going to chat');
                this.startChat();
            });
        }

        // Learn more button - show more details about CURA
    ...
    startChat() {
        console.log('🚀 Starting chat directly...');
        
        // Set a default user for the chat (since we removed authentication)
        this.setUser({
            username: 'User',
            email: 'user@cura.ai',
            first_name: 'Cura',
        ...
        this.notifications.success('Welcome to CURA! How can I help you today?', {
            title: 'Ready to Chat',
            duration: 3000
        });
    ...
    showAuthInterface() {
        console.log('🔧 showAuthInterface called');
        if (this.chatSection) {
    ...
    showMoreInfo() {
        // Scroll down to features or show expanded information
        const featuresSection = document.querySelector('.feature-showcase');
        if (featuresSection) {
            featuresSection.scrollIntoView({ behavior: 'smooth' });
        }
        
        // Add some visual feedback
        this.notifications.info('Explore CURA\'s features below! Ready to get started?', {
    ...
    enterGuestMode() {
        ...
        // Set guest token and user data
        this.api.setGuestToken();
        this.setUser({
            username: 'Guest User',
            email: 'guest@cura.ai',
            first_name: 'Guest',
        ...
        // Show success message
        this.notifications.success('Welcome! You\'re now using CURA in guest mode.', {
    ...
    updateUserGreeting() {
        ...
        if (userGreeting && this.state.user) {
            ...
            if (this.state.user.isGuest) {
                userGreeting.textContent = `${greeting}! Welcome to CURA`;
            } else {
    ...
    updateWelcomeMessage() {
        if (this.state.user && this.state.user.isGuest) {
            ...
            if (welcomeTitle) {
                welcomeTitle.textContent = '👋 Welcome to CURA! I\'m your AI medical assistant.';
            }
            if (welcomeDescription) {
                welcomeDescription.innerHTML = `You're exploring CURA in guest mode! While you can see the interface, 
                    <strong>you'll need to create an account</strong> to unlock the full AI chat capabilities. 
                    I can help you with:`;
            }
            if (welcomePrompt) {
                welcomePrompt.innerHTML = `
                    ...
                    <small style="color: rgba(255,255,255,0.7);">
                        Create a free account to get personalized medical guidance and save your conversation history.
                    </small>
                `;
            }
        }
